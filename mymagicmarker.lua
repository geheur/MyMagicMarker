...
--g_tblMobs = {}
tblGuids = {} -- TODO make local again.
...
local function calculateDesiredMarks(name)
	local count = 1
	if true--[[TODO dynamic marking]] then
		for i=1,#tblGuids[name].instances do
			local unitData = tblGuids[name].instances[i]
			if unitData.alive then
				local mark = g_tblMobs[name].marks[count]
				if mark == nil then mark = NONE end
				--print("Setting mark", count, mark)
				unitData.mark = mark
				count = count + 1
			end
		end
...
local function GetUnitData(name, guid)
	if tblGuids[name] == nil then return nil end
	for i,v in ipairs(tblGuids[name].instances) do
		if v.guid == guid then
			return v
		end
	end
...
local function GetOrCreateUnitData(name, guid)
   if tblGuids[name]==nil then
		tblGuids[name] = {}
		tblGuids[name].instances = {}
   end

	local unitData = GetUnitData(name, guid)
	if unitData ~= nil then return unitData, false end

	unitData = {guid=guid, mark=NONE, alive=true}
	tblGuids[name].instances[#tblGuids[name].instances + 1] = unitData
...
local function HandleUnit (strunit)
    ...
    if g_tblMobs[name] and NeedToMark(strunit, name) then
		  local unitData, created = GetOrCreateUnitData(name, guid)

		  if created then
			  calculateDesiredMarks(name)
		  end

		  if GetRaidTargetIndex(strunit) ~= unitData.mark then
		  	_G.SetRaidTarget(strunit, unitData.mark)
		  end
    end
end

local function UpdateNameplateTargets()
	for i=0,20 do
		local nameplateUnit = "nameplate"..i
		if UnitExists(nameplateUnit) then
			HandleUnit(nameplateUnit)
		end
	end
end

local function UpdateGroupMemberTargets()
	for i=1,_G.GetNumGroupMembers() do
...
local function HandleDeadUnit(name, guid)
	 ...
	 -- update immediately wherever possible.
	 UpdateGroupMemberTargets()
	 UpdateNameplateTargets()
end
...
frm:RegisterEvent ("NAME_PLATE_UNIT_ADDED")
frm:RegisterEvent ("COMBAT_LOG_EVENT_UNFILTERED")
frm:SetScript ("OnEvent", function  (self, event, ...)
    if event=='PLAYER_REGEN_DISABLED' then          -- Player enters combat
        combatStarted = true
        if _G.IsInRaid() then
            groupType = 'raid'
        else
    ...
    elseif event=='COMBAT_LOG_EVENT_UNFILTERED' then
	 	  local _,eventType,_,_,_,_,_,guid,name = CombatLogGetCurrentEventInfo()
	 	  if eventType == "UNIT_DIED" then
			   HandleDeadUnit(name:lower(), guid)
		  else
